...
function homeView() {
  ...
  return `<div class="dashboard-grid"><section><div class="section-head"><h2>Community feed</h2><span>${state.posts.length} uploaded ${state.posts.length === 1 ? "work" : "works"}</span></div><div class="feed">${feed}</div></section><aside class="side-col"><div class="side-card"><h3>Lesotho in focus</h3><p style="color:var(--muted);font-size:.8rem;line-height:1.5">News and opportunities will appear here when they are published by PAUL APPORG.</p><button class="text-btn" data-view="news">Open newsroom →</button></div><div class="side-card"><h3>Quick converter</h3><p style="color:var(--muted);font-size:.78rem;margin-top:-8px">Guide rate: 1 GBP ≈ 23.50 LSL</p><div class="converter-row"><input id="amount" type="number" value="100" min="0"><select id="currency"><option value="lsl">Maloti → GBP</option><option value="gbp">GBP → Maloti</option></select></div><div id="conversion-result" class="conversion-result">≈ £4.26</div></div>${state.user.role === "artist" ? '<div class="side-card"><h3>Share your work</h3><p style="color:var(--muted);font-size:.8rem;line-height:1.5">Have something new for the community?</p><button id="open-upload" class="primary-btn upload-btn">＋ Upload artwork</button></div>' : ""}</aside></div>`;
}
