...
function Auth() {
  ...
  return <main className="auth-page"><div className="auth-brand"><div className="brand-mark">f</div><h1>Connect with the people and things you love.</h1><p>Share moments, discover creative work, and keep up with your community.</p></div><form onSubmit={submit} className="auth-card"><h2>{register ? "Create an account" : "Welcome back"}</h2><p className="muted">{register ? "Join your community today." : "Sign in to see what’s happening."}</p>{register && <input className="field" required placeholder="Full name" value={name} onChange={e => setName(e.target.value)} />}<input className="field" required type="email" placeholder="Email address" value={email} onChange={e => setEmail(e.target.value)} /><input className="field" required minLength={6} type="password" placeholder="Password" value={password} onChange={e => setPassword(e.target.value)} />{error && <p className="error">{error}</p>}<button className="primary wide">{register ? "Create account" : "Log in"}</button><div className="divider"><span>or</span></div><button type="button" className="text-button" onClick={() => setRegister(!register)}>{register ? "Already have an account? Log in" : "Create new account"}</button></form></main>;
}

function Navbar({ user }: { user: SessionUser }) {
  const [search, setSearch] = useState("");
  return <header className="topbar"><div className="topbar-inner"><Link to="/" className="brand"><span className="brand-mark">f</span><span className="brand-word">facebook</span></Link><label className="search"><Icon>⌕</Icon><input aria-label="Search" placeholder="Search Facebook" value={search} onChange={e => setSearch(e.target.value)} /></label><nav className="top-nav" aria-label="Primary navigation"><Link className="nav-link active" to="/" aria-label="Home"><Icon>⌂</Icon><span>Home</span></Link><Link className="nav-link" to="/popular" aria-label="Discover"><Icon>▣</Icon><span>Discover</span></Link></nav><div className="top-actions"><button className="circle-button" aria-label="Create post">＋</button><button className="circle-button" aria-label="Messages">♧</button><button className="circle-button" aria-label="Notifications">♡</button><Link to={`/profile/${user.uid}`} className="profile-chip"><Avatar src={user.photoURL || undefined} name={user.displayName || "You"} size="avatar-sm" /><span>{user.displayName?.split(" ")[0] || "You"}</span></Link><button className="logout" onClick={() => signOut(auth)}>Log out</button></div></div></header>;
}

function LeftSidebar({ profile }: { profile: Profile }) {
  return <aside className="left-sidebar"><Link className="sidebar-profile" to={`/profile/${profile.uid}`}><Avatar src={profile.photoURL} name={profile.displayName} /><strong>{profile.displayName}</strong></Link><nav className="side-nav"><Link className="side-item active" to="/"><Icon>⌂</Icon> Home</Link><Link className="side-item" to="/popular"><Icon>♧</Icon> Friends</Link><Link className="side-item" to="/popular"><Icon>▣</Icon> Groups</Link><Link className="side-item" to="/popular"><Icon>▤</Icon> Marketplace</Link><Link className="side-item" to={`/profile/${profile.uid}`}><Icon>◉</Icon> Memories</Link></nav><div className="sidebar-divider" /><p className="sidebar-label">Your shortcuts</p><div className="shortcut"><span className="shortcut-icon purple">✦</span><span>Creative community</span></div><div className="shortcut"><span className="shortcut-icon orange">◒</span><span>Weekend runners</span></div><div className="shortcut"><span className="shortcut-icon green">♧</span><span>Design thinkers</span></div></aside>;
}
...
function Feed({ user }: { user: SessionUser }) {
  ...
  return <><Navbar user={user} /><div className="app-layout"><LeftSidebar profile={profile} /><main className="feed"><div className="feed-heading"><div><p className="eyebrow">YOUR FEED</p><h1>Good morning, {profile.displayName.split(" ")[0]} <span>👋</span></h1><p className="muted">Here’s what’s happening in your community.</p></div><button className="filter-button">Recent <span>⌄</span></button></div><Stories user={user} /><Composer profile={profile} onLocalPost={addLocalPost} />{posts.map(post => <PostCard key={post.id} post={post} uid={user.uid} />)}</main><RightSidebar /></div></>;
}
...
function App() {
  ...
  if (!user) return <Auth />;
  return <><div className="demo-banner">Demo mode · Connect Firebase environment variables to enable accounts and live data.</div><Routes><Route path="/profile/:uid" element={<ProfilePage user={user} />} /><Route path="/popular" element={<PopularArtists user={user} />} /><Route path="*" element={<Feed user={user} />} /></Routes></>;
}
